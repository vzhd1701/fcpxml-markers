...
META_NAMESPACE = "fcpx_xml_custom"
...
class FcpxOtio(object):
    ...
    def __init__(self, otio_timeline):
        ...
        if self.otio_timeline.schema_name() == "Timeline":
            ...
            self.timelines = list(
                self.otio_timeline.each_child(descended_from_type=otio.schema.Timeline)
            )

        if len(self.timelines) > 1:
            self.event_resource = cElementTree.SubElement(
                self.fcpx_xml, "event", {"name": self._event_name()}
            )
        else:
    ...
    def to_xml(self):
        ...
        if not self.timelines:
            ...
            for stack in self._stacks():
                ref_element = self._element_for_item(
                    stack, None, ref_only=True, compound=True
                )
        ...
        for marker in [marker for marker in self.fcpx_xml.iter("marker")]:
            ...
            cElementTree.SubElement(parent, "marker", marker_attribs)
        ...
        sequence_element = cElementTree.Element(
            "sequence",
            {
                "duration": self._calculate_rational_number(
                    stack.duration().value, stack.duration().rate
                ),
                "format": str(format_element.get("id")),
            },
        )
        spine = cElementTree.SubElement(sequence_element, "spine")
        video_tracks = [t for t in stack if t.kind == otio.schema.TrackKind.Video]
        audio_tracks = [t for t in stack if t.kind == otio.schema.TrackKind.Audio]
    ...
    def _track_for_spine(self, track, lane_id, spine, compound):
        for child in self._lanable_items(track.each_child()):
            if self._item_in_compound_clip(child) and not compound:
                continue
            child_element = self._element_for_item(child, lane_id, compound=compound)
            if not lane_id:
            ...
            child_element.set("offset", from_rational_time(offset))
    ...
    def _find_parent_element(self, spine, trimmed_range, format_id):
        for item in spine.iter():
            ...
            offset = to_rational_time(
                item.get("offset"), self._frame_rate_from_element(item, format_id)
            )
            duration = to_rational_time(
                item.get("duration"), self._frame_rate_from_element(item, format_id)
            )
    ...
    def _offset_based_on_parent(self, child, parent, default_format_id):
        ...
        child_offset = to_rational_time(
            child.get("offset"), self._frame_rate_from_element(child, default_format_id)
        )
    ...
    def _element_for_item(self, item, lane, ref_only=False, compound=False):
        element = None
        duration = self._calculate_rational_number(
            item.duration().value, item.duration().rate
        )
        ...
        for marker in item.markers:
            ...
            marker_element = cElementTree.Element("marker", marker_attribs)
    ...
    def _lanable_items(self, items):
        return [
            item for item in items if item.schema_name() in ["Gap", "Stack", "Clip"]
        ]
    ...
    def _element_for_clip(self, item, asset_id, duration, lane):
        element = cElementTree.Element(
            ...
            {
                "name": item.name,
                "offset": from_rational_time(item.trimmed_range_in_parent().start_time),
                "duration": duration,
            },
        )
    ...
    def _element_for_stack(self, item, duration, ref_only):
        ...
        if not ref_only:
            ...
            element.set("start", from_rational_time(item.source_range.start_time))
    ...
    def _find_asset_duration(self, item):
        if item.media_reference and not item.media_reference.is_missing_reference:
    ...
    def _find_asset_start(self, item):
        ...
        return self._calculate_rational_number(
            item.source_range.start_time.value, item.source_range.start_time.rate
        )
    ...
    def _clip_format_name(self, clip):
        ...
        return format_name(clip.duration().rate, clip.media_reference.target_url)

    def _find_or_create_format_from(self, clip):
        frame_duration = self._framerate_to_frame_duration(clip.duration().rate)
    ...
    def _add_asset(self, clip, compound_only=False):
        ...
        if not compound_only and not self._asset_clip_by_name(clip.name):
            self._create_asset_clip_element(clip, format_element, asset.get("id"))
    ...
    def _create_asset_clip_element(self, clip, format_element, resource_id):
        ...
        if clip.media_reference and not clip.media_reference.is_missing_reference:
            ...
            note_element = self._create_note_element(fcpx_metadata.get("note", None))
    ...
    def _add_compound_clip(self, item):
        ...
        media_element = cElementTree.SubElement(
            self.resource_element,
            "media",
            {"name": self._compound_clip_name(item, resource_id), "id": resource_id},
        )
    ...
    def _stacks(self):
        return self.otio_timeline.each_child(descended_from_type=otio.schema.Stack)

    def _clips(self):
        return self.otio_timeline.each_child(descended_from_type=otio.schema.Clip)
    ...
    def _asset_clip_by_name(self, name):
        return self.event_resource.find("./asset-clip[@name='{}']".format(name))
    ...
    def _create_keyword_elements(keywords):
        ...
        for keyword_dict in keywords:
            keyword_elements.append(cElementTree.Element("keyword", dict(keyword_dict)))
    ...
    def _create_note_element(note):
        if not note:
            return None
        note_element = cElementTree.Element("note")
...
class FcpxXml(object):
    ...
    def _from_event(self, event_element):
        container = otio.schema.SerializableCollection(name=event_element.get("name"))
    ...
    def _from_project(self, project_element):
        ...
        timeline.tracks = self._squence_to_stack(project_element.find("./sequence", {}))
        return timeline
    ...
    def _from_clips(self):
        ...
        if self.fcpx_xml.find("./asset-clip") is not None:
            for asset_clip in self.fcpx_xml.findall("./asset-clip"):
                container.append(
                    self._build_composable(asset_clip, asset_clip.get("format"))
                )

        if self.fcpx_xml.find("./ref-clip") is not None:
            for ref_clip in self.fcpx_xml.findall("./ref-clip"):
                container.append(self._build_composable(ref_clip, "r1"))
        return container
    ...
    def _squence_to_stack(self, sequence_element, name="", source_range=None):
        ...
        for element in sequence_element.iter():
            if element.tag not in COMPOSABLE_ELEMENTS:
                continue
            composable = self._build_composable(element, sequence_element.get("format"))

            offset, lane = self._offset_and_lane(
                element, sequence_element.get("format")
            )
        ...
        for lane in sorted_lanes:
            ...
            track = otio.schema.Track(name=lane, kind=self._track_type(sorted_items))

            for item in sorted_items:
                frame_diff = int(item["offset"].value) - track.duration().value
                if frame_diff > 0:
                    track.append(
                        self._create_gap(0, frame_diff, sequence_element.get("format"))
                    )
    ...
    def _build_composable(self, element, default_format):
        ...
        source_range = self._time_range(
            timing_clip, self._format_id_for_clip(element, default_format)
        )
        ...
        if element.tag != "ref-clip":
            otio_composable = otio.schema.Clip(
                ...
                media_reference=self._reference_from_id(
                    element.get("ref"), default_format
                ),
    ...
    def _offset_and_lane(self, clip, default_format):
        ...
        clip_offset_frames = self._number_of_frames(clip.get("offset"), clip_format_id)

        if spine:
            parent_start_frames = 0
        else:
            parent_start_frames = self._number_of_frames(
                parent.get("start", None), parent_format_id
            )

        parent_offset_frames = self._number_of_frames(
            parent.get("offset", None), parent_format_id
        )

        clip_offset_frames = (int(clip_offset_frames) - int(parent_start_frames)) + int(
            parent_offset_frames
        )

        offset = otio.opentime.RationalTime(
            clip_offset_frames, self._format_frame_rate(clip_format_id)
        )
    ...
    def _format_id_for_clip(self, clip, default_format):
        ...
        if resource is None:
            resource = self._compound_clip_by_id(clip.get("ref")).find("sequence")
    ...
    def _reference_from_id(self, asset_id, default_format):
        ...
        if asset.get("src"):
            ...
            asset_media = self._asset_media_by_id(asset_id)
            if not asset_media.get("src"):
                return otio.schema.MissingReference()
            media_src = asset_media.get("src")

        available_range = otio.opentime.TimeRange(
            start_time=to_rational_time(
                asset.get("start"),
                self._format_frame_rate(asset.get("format", default_format)),
            ),
            duration=to_rational_time(
                asset.get("duration"),
                self._format_frame_rate(asset.get("format", default_format)),
    ...
    def _number_of_frames(self, time_value, format_id):
        ...
        return int(
            int(time_value[0].replace("s", "")) * (float(fd_rate) / float(fd_total))
        )

    def _time_range(self, element, format_id):
        return otio.opentime.TimeRange(
            start_time=to_rational_time(
                element.get("start", "0s"), self._format_frame_rate(format_id)
            ),
            duration=to_rational_time(
                element.get("duration"), self._format_frame_rate(format_id)
            ),
        )
    ...
    def _asset_by_id(self, asset_id):
        return self.fcpx_xml.find("./resources/asset[@id='{}']".format(asset_id))

    def _asset_media_by_id(self, asset_id):
        return self.fcpx_xml.find(
            "./resources/asset[@id='{}']/media-rep".format(asset_id)
        )
    ...
    def _format_by_id(self, format_id):
        return self.fcpx_xml.find("./resources/format[@id='{}']".format(format_id))

    def _compound_clip_by_id(self, compound_id):
        return self.fcpx_xml.find("./resources/media[@id='{}']".format(compound_id))
